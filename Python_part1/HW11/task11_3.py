...
size = float(input('Ведите размер файла обновления в мегабайтах: '))
speed = float(input('Ведите корость интернет соединения в мегабайтах в секунду: '))
n = int(size // speed)

for sec in range(1, n + 1):
    loaded = speed * sec
    proportion = int(loaded / size * 100)
    print(f'Прошло {sec} сек. Скачано {loaded} из {size} Мб {proportion}% \n'.format())

if size % speed != 0:
    print(f'Прошло {sec + 1} сек. Скачано {size} из {size} Мб 100%'.format())
